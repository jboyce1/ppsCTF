...
# Function to get the lowest IP from ARP table
...
# Function to detect the network subnet using ARP + IP
...
# Function to send ICMP bursts in waves
def send_icmp_broadcast(live_hosts, burst_num):
    ...
    for host in live_hosts:
        for _ in range(BURST_COUNT):
            send(IP(dst=host)/ICMP()/Raw(load="A" * PACKET_SIZE), verbose=0)
