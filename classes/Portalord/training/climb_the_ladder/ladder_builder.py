...
def write_flag_text(flag: str) -> Path:
    p = desktop_path() / FLAG_TXT_NAME
    p.write_text(flag + "\n", encoding="utf-8")
    os.chmod(p, 0o600)
    return p
...
def create_restricted_shell_script() -> str:
    ...
    # Keep PATH tight so rbash matters for your classroom constraint
    contents = """#!/bin/bash
echo "Limited access."
...
def ensure_pillow():
    # Prefer apt so it works in restricted environments
    try:
...
def write_flag_image_pixels(flag: str) -> Path:
    """
    Draw the flag into pixels (not metadata) so 'strings flag.png' on the box
    won't reveal it. Students must SCP it out and inspect visually.
    """
    ensure_pillow()
...
def start_udp_stream(prev_ip: str, port: int, message: str):
    """
    Sends UDP packets forever. No connection required.
    Great for FIFO/wireshark capture on previous box.
    """
    # avoid quoting problems by embedding in python heredoc
    py = f"""python3 - <<'PY'
import socket, time
ip={prev_ip!r}
port={port}
msg={message!r}.encode()
...
def main():
    ...
    print("\n=== Portalord: Versatile Node Setup ===\n")
    print("Access type:")
    print("  1) open_to_all")
    ...
    print("\nBeacon type:")
    print("  1) icmp")
    print("  2) udp_stream (includes flag)")
    print("  3) none")
    beacon = input("Choose [1/2/3]: ").strip()

    # Generate flag once for this box
    flag = generate_flag()
    print(f"[*] Flag generated (store this in your build notes): {flag}")
    ...
    if access == "1":
        ufw_allow_ssh_any()
        ufw_enable()
        p = write_flag_text(flag)
        print(f"[*] Desktop flag file: {p}")

    elif access == "2":
        install_telnet_services()
        ufw_allow_telnet_from(prev_ip)
        # SSH remains blocked by default deny incoming
        ufw_enable()
        ...
        ufw_enable()
        p = write_flag_image_pixels(flag)
        print(f"[*] Desktop flag image (pixels): {p}")
        print("    Students must SCP the image out and visually read the flag.")
    ...
    if beacon == "1":
        ...
        if not prev_ip:
            print("Beacon requires Previous node IP (choose access 2 or 3).")
            sys.exit(1)
        port_str = input("UDP port to send to on previous node [5555]: ").strip() or "5555"
        port = int(port_str)

        # include flag in UDP payload
        msg = f"PORTALORD_UDP_BEACON|{flag}|{time.time():.0f}"
        start_udp_stream(prev_ip, port, msg)
