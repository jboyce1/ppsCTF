...
# Configuration
TAR_FILE = "Bully2.tar.gz"  # Assumed to be in the same directory as the script
...
def create_users():
    for user, password in USER_CREDENTIALS.items():
        ...
        # Grant sudo privileges to all users
...
def distribute_files():
    base_extracted_path = os.path.join(EXTRACT_PATH, "Bully2")
    for user in USER_CREDENTIALS.keys():
...
def main():
    ...
    setup_ssh()
    distribute_files()
    print("[✅] Bully2 setup complete! Ready for CTF deployment.")
