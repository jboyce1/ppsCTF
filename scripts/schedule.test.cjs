...
test('meeting is ongoing from its exact start until its end, then advances', () => {
  assert.equal(at('2026-10-22T18:59:59-04:00').status, 'Next meeting');
  assert.equal(at('2026-10-22T19:00:00-04:00').status, 'Ongoing now');
  assert.equal(at('2026-10-22T20:59:59-04:00').event.id, 'session-10-22');
  assert.equal(at('2026-10-22T21:00:00-04:00').event.id, 'session-10-27');
});
test('Eastern standard time is respected after the DST change', () => {
  assert.equal(at('2026-11-05T23:59:59Z').status, 'Next meeting');
  assert.equal(at('2026-11-06T00:00:00Z').status, 'Ongoing now');
  assert.equal(at('2026-11-06T02:00:00Z').event.id, 'session-11-10');
});
...
test('scheduled closeout takes precedence over the overlapping competition window', () => {
  ...
  assert.equal(at('2026-12-23T19:00:00-05:00').event.id, 'session-12-23');
  assert.equal(at('2026-12-23T19:00:00-05:00').status, 'Ongoing now');
});
